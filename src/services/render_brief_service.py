...
class RenderBriefBuilder:
    def build(self, state: dict[str, Any], output_filename: str, voice_filename: str) -> dict[str, Any]:
        ...
        return {
            "script": self._script_payload(state, script_data, script_section),
            "voice_plan": self._voice_plan(state, script_data),
            "video_assets": self._video_assets(state.get("video_paths", []), media_plan),
            "music_asset": {
    ...
    def _voice_plan(self, state: dict[str, Any], script_data: dict[str, Any]) -> dict[str, Any]:
    ...
    def _video_assets(self, video_paths: list[str], media_plan: dict[str, Any]) -> list[dict[str, Any]]:
        video_scenes = list(media_plan.get("video_scenes") or [])
        assets = []
        for index, path in enumerate(video_paths, start=1):
            scene = self._scene_at(video_scenes, index)
            assets.append(
                {
    ...
    def _scene_at(self, video_scenes: list[Any], index: int) -> dict[str, Any]:
        scene_index = index - 1
...
def build_render_brief(state: dict[str, Any], output_filename: str, voice_filename: str) -> dict[str, Any]:
    return RenderBriefBuilder().build(state, output_filename, voice_filename)
